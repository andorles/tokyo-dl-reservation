...
class EmailNotifier:
    ...
    def booked(
        ...
        body = (
            ...
            f"  試験場       : {place_name} (placecode={slot.place})\n"
            f"  免許種別     : {course_name} (coursecode={slot.course})\n"
            f"  受付番号     : {slot.receipt_no or '(not returned by upstream)'}\n"
            f"  予約成立時刻 : {slot.booked_at}\n\n"
            "📋 上流 response 内の ID 系フィールド "
            "(予約番号 / 受付番号 / yoyaku_no 等を網羅的に拾ったもの):\n"
            f"{id_summary}\n\n"
            "添付ファイル / Attachments:\n"
            f"{attachment_summary}\n\n"
            "次のステップ:\n"
            "  1. 当日試験場で受付番号 / QR を提示 (添付の qr*.png を保存しておく)\n"
            "  2. 予約日に試験場へ持っていくべき書類を再確認\n"
            "  3. 当日の集合時間(受付時間)を再確認\n"
            f"  4. 不要になった場合: スクリプトは予約を取消ししません — "
            f"自分で {_BOOKING_URL} の「予約キャンセル」フローから取消し、\n"
            "     かつ `dl-poll --reset-booking` で booker を再アクティベート\n"
        )
        msg = self._compose(
            subject=(
